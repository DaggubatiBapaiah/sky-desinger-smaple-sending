...
# Parsing logic to construct categoryDetails object
data = {
  'visiting-cards': [
...
# we don't have polo-shirts category separately in this extracted map, but it's okay, some apparel can overlap or we can just leave polo-shirts as it was or remove it from categories. Wait, I'll add a polo-shirts key with just basic.

with open('scratch/out.txt', 'w') as f:
    f.write("export const categoryDetails: Record<string, {name: string; description: string; image: string}[]> = {\n")
    for cat_id, items in data.items():
        f.write(f"  '{cat_id}': [\n")
        for item in items:
            f.write(f"    {{ name: \"{item}\", description: \"Explore options for {item}\", image: \"https://images.pexels.com/photos/5650018/pexels-photo-5650018.jpeg\" }},\n")
        f.write("  ],\n")
    f.write("};\n")
